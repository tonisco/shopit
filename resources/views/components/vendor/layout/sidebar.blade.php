<nav class="flex flex-col h-full pt-6 text-gray-800 bg-gray-100 dark:text-gray-200 dark:bg-gray-800">
    ...
    <a href="{{ route('vendor.orders.index') }}"
        class="flex items-center gap-2 px-6 py-4 transition-all cursor-pointer hover:text-white hover:bg-red-500 dark:hover:bg-red-700 active:text-white active:bg-red-500 active:hover:bg-red-700 {{ request()->routeIs('vendor.orders.index') ? 'text-white bg-red-500 dark:bg-red-700' : '' }}">
        <x-ri-clipboard-line class="w-5 h-5" />
        <p> Orders </p>
    </a>
    <a href="{{ route('vendor.products.index') }}"
        class="flex items-center gap-2 px-6 py-4 transition-all cursor-pointer hover:text-white hover:bg-red-500 dark:hover:bg-red-700 active:text-white active:bg-red-500 active:hover:bg-red-700 {{ request()->routeIs('vendor.product*') ? 'text-white bg-red-500 dark:bg-red-700' : '' }}">
