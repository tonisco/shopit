<x-vendor.layout.main page="Products Variant Item">
    <section class="flex flex-col flex-1 gap-6 px-4 py-8">
        <div class="flex flex-col items-start justify-between w-full gap-2 sm:flex-row sm:items-center">
            <x-vendor.layout.heading title="{{ $productVariant->name }} Variant" :crumbs="[
                ['name' => 'products', 'route' => route('vendor.products.index')],
                ['name' => 'variant', 'route' => route('vendor.products.variants.index', $productVariant->product->id)],
                ['name' => 'variant item'],
            ]"
                subtitle="product: <span class='font-medium'>{{ $productVariant->product->name }}</span>" />
            <a href="{{ route('vendor.products.variants.items.create', ['product' => $productVariant->product->id, 'variant' => $productVariant->id]) }}"
            ...
            <table class="!w-full datatable text-gray-800 dark:text-gray-200">
                <thead>
                    <tr class="font-semibold capitalize">
                        ...
                        <td class="px-1 py-2 text-xs md:text-sm"> isDefault </td>
        ...
        <script type="text/javascript">
            ...
            $(function() {
                let table = $('.datatable').DataTable({
                    ...
                    serverside: true,
                    ajax: {
                        url: "{{ route('vendor.products.variants.items.index', ['product' => $productVariant->product->id, 'variant' => $productVariant->id]) }}"
                    },
                    columns: [{
                            data: 'id'
                        },
                    ...
                    drawCallback: function(settings, ) {
                        ...
                        $('.status').on('click', function(e) {
                            ...
                            if (e.target.checked) {
                                let message = `Are you sure you want to set
								 <span class="font-semibold capitalize">${name}</span> to default`
                                $('.check-heading').text(message)
                                $('.check-button').text('Show')
                            } else {
                                let message = `Are you sure you want to remove
								 <span class="font-semibold capitalize">${name}</span> from default`
